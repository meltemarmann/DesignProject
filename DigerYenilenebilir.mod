 ...
 * OPL 22.1.0.0 Model
 * Author: MeltemArman
 * Creation Date: 6 Nis 2023 at 13:22:11
 *********************************************/
...
float y[T] = ...; //Diger yenilenebilir kaynaklarin toplam saatlik uretimi.
...
forall(t in T){
	g_pv[t]*M_pv + g_w[t]*M_w + X_dis[t]*n + y[t] >= D[t] + X_ch[t]/n; //Akis dengeleme ve talep karsilama kisiti.
}
forall(t in T){
